...
#[pgrx::pg_guard]
pub unsafe extern "C-unwind" fn ambuildempty(index: pgrx::pg_sys::Relation) {
    let mut meta_page = unsafe { page_alloc_init_forknum(index, PageFlags::META) };
    assert_eq!(meta_page.blkno(), METAPAGE_BLKNO);

    let meta: &mut MetaPageData = unsafe { meta_page.init_mut() };
    meta.field_norm_blkno = unsafe { VirtualPageWriter::init_fork(index, PageFlags::FIELD_NORM) };
    meta.term_stat_blkno =
        unsafe { VirtualPageWriter::init_fork(index, PageFlags::TERM_STATISTIC) };
    meta.payload_blkno = unsafe { VirtualPageWriter::init_fork(index, PageFlags::PAYLOAD) };
    meta.delete_bitmap_blkno = unsafe { VirtualPageWriter::init_fork(index, PageFlags::DELETE) };
}
...
pub unsafe extern "C-unwind" fn ambuild(
    ...
    {
        let metapage = unsafe { page_alloc(index, PageFlags::META, true) };
...
fn write_down(state: &BuildState) {
    ...
    let mut meta_page = unsafe { page_write(state.index, METAPAGE_BLKNO) };
    let meta: &mut MetaPageData = unsafe { meta_page.init_mut() };
    ...
    let mut delete_bitmap_writer =
        unsafe { VirtualPageWriter::new(state.index, PageFlags::DELETE, true) };
    ...
    // term stat
    let mut term_stat_writer =
        unsafe { VirtualPageWriter::new(state.index, PageFlags::TERM_STATISTIC, true) };
    ...
    let (payload_blkno, field_norm_blkno, sealed_data) =
        unsafe { state.builder.serialize(state.index) };
