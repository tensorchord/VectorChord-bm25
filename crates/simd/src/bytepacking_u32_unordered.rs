...
pub fn compress(bytewidth: u8, input: &[u32], output: &mut [u8]) {
    assert!(
    ...
    match bytewidth {
        1 => compress_1::compress(input, output),
        2 => compress_2::compress(input, output),
        3 => compress_3::compress(input, output),
        4 => output.copy_from_slice(zerocopy::IntoBytes::as_bytes(input)),
        _ => panic!("bytewidth out of bound"),
    }
}

pub fn decompress(bytewidth: u8, input: &[u8], output: &mut [u32]) {
    assert!(
        matches!(bytewidth, 1..=4)
            && output.len() <= 128
    ...
    match bytewidth {
        1 => decompress_1::decompress(input, output),
        2 => decompress_2::decompress(input, output),
        3 => decompress_3::decompress(input, output),
