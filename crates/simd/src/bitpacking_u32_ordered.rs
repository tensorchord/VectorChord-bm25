    ...
    mod decompress_~BITWIDTH {
        ...
        fn decompress_v2(min: u32, input: &[u8], output: &mut [u32; 128]) {
            ...
            fn delta(state: &mut S, value: T) -> T {
                ...
                let a___b___c___d___ = value;
                let ________a___b___ = _mm_slli_si128(value, 8);
                let a___b___ac__bd__ = _mm_add_epi32(a___b___c___d___, ________a___b___);
                let ____a___b___ac__ = _mm_slli_si128(a___b___ac__bd__, 4);
                let a___ab__abc_abcd = _mm_add_epi32(a___b___ac__bd__, ____a___b___ac__);
                let result = _mm_add_epi32(base, a___ab__abc_abcd);
                *state = result;
                result
            }
            use core::arch::x86_64::_mm_or_si128 as bitor;
        ...
        fn decompress_a2(min: u32, input: &[u8], output: &mut [u32; 128]) {
            ...
            fn delta(state: &mut S, value: T) -> T {
                ...
                let a___b___c___d___ = value;
                let ________a___b___ = vextq_u32(zero, a___b___c___d___, 2);
                let a___b___ac__bd__ = vaddq_u32(a___b___c___d___, ________a___b___);
                let ____a___b___ac__ = vextq_u32(zero, a___b___ac__bd__, 3);
                let a___ab__abc_abcd = vaddq_u32(a___b___ac__bd__, ____a___b___ac__);
                let result = vaddq_u32(base, a___ab__abc_abcd);
                *state = result;
                result
            }
            use core::arch::aarch64::vorrq_u32 as bitor;
